...
TEST_CASE("timed_op_wrapper_v3")
{
  ...
  auto session = launchdarkly::foxy::session(io, launchdarkly::foxy::session_opts{});

  auto impl = [&session](auto& self, boost::system::error_code ec = {},
                         std::size_t bytes_transferred = 0) mutable { self.complete({}, 0); };

  launchdarkly::foxy::detail::async_timer<void(boost::system::error_code ec, std::size_t bytes_transferred)>(
