...
TEST_CASE("code_point_view_test")
{
  SECTION("should interop with our Unicode URI parsers")
  {
    ...
    auto point_view = launchdarkly::foxy::code_point_view<char16_t>(url);
    ...
    auto const uri_parts = launchdarkly::foxy::parse_uri(unicode_url);
  ...
  SECTION("[wide] should interop with our utf-8 encoder")
  {
    ...
    auto point_view = launchdarkly::foxy::code_point_view<wchar_t>(input);
    auto buff       = std::array<unsigned char, 3>{0};

    launchdarkly::foxy::utf8_encode(point_view.begin(), point_view.end(), buff.begin());
  ...
  SECTION("[utf-32] should interop with our utf-8 encoder")
  {
    ...
    auto point_view = launchdarkly::foxy::code_point_view<char32_t>(input);
    auto buff       = std::array<unsigned char, 3>{0};
  ...
  SECTION("[utf-16] should interop with our utf-8 encoder")
  {
    ...
    auto point_view = launchdarkly::foxy::code_point_view<char16_t>(input);
    auto buff       = std::array<unsigned char, 3>{0};
  ...
  SECTION("[utf-8] should interop with our utf-8 encoder")
  {
    ...
    auto point_view = launchdarkly::foxy::code_point_view<char>(input);
    auto buff       = std::array<unsigned char, 3>{0};
