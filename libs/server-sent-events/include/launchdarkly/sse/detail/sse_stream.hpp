...
class sse_stream {
    ...
    void decode_and_buffer_lines(MutableBufferSequence const& chunk) {
        ...
        std::vector<std::string> tokens{std::begin(tokens_all), std::end(tokens_all)};
...
public:
    ...
    /// Read some data from the stream
    template <class MutableBufferSequence>
    std::size_t read_some(MutableBufferSequence const& buffers, beast::error_code& ec)
    {
