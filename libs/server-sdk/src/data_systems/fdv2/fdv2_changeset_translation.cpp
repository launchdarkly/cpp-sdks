        ...
        if (change.change_type == data_model::FDv2Change::ChangeType::kDelete) {
            if (change.kind == "flag") {
                changes.push_back(ItemChange{
                    change.key, data_model::ItemDescriptor<data_model::Flag>{
                                    data_model::Tombstone{change.version}}});
            } else if (change.kind == "segment") {
                changes.push_back(ItemChange{
                    change.key, data_model::ItemDescriptor<data_model::Segment>{
                                    data_model::Tombstone{change.version}}});
            } else {
            ...
            if (change.kind == "flag") {
                ...
                auto result = boost::json::value_to<tl::expected<
                    std::optional<data_model::Segment>, JsonError>>(
                    change.object);
                if (!result) {
                    LD_LOG(logger, LogLevel::kError)
